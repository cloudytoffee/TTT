...
with open(r'source/include/options.json', 'r') as c:
    constants = json.load(c)  # turn the json into a list


# the two needed assests (X and O images)
x_letter = pg.image.load(os.path.join("source", "include",
                                      "data", "x_letter.gif"))
o_letter = pg.image.load(os.path.join("source", "include",
                                      "data", "o_letter.gif"))

# the text blocks for the about window
ABOUT = [f'This is a simple GUI based two player TIC TAC TOE game',
         ...
         f'with varying difficulty levels,',
         f'but my procrastination and work ethic',
         f'along with a large amount of stress from lockdown and exams',
         f'made for a very mediocre result',
         f'Anyway, just click on the squares',
         f'like you would usually play TicTacToe',
